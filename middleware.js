...
export async function middleware (request) {
  ...
  if(verifiedToken == null) {
    ...
    //skip token for /api/login
    //not skipping for /api/signup, as only logged in admin can create client users
    if(path === "/api/login") {
  ...
  if(path.startsWith("/admin") && isAdmin_) {
    ...
    if(isAdmin_) {
      console.log("/client is blocked. Redirecting to /admin")
      return NextResponse.redirect(new URL("/admin", request.nextUrl));
    } else {
      console.log("/client is blocked. Redirecting to / ")
      return NextResponse.redirect(new URL("/", request.nextUrl));
    } 
    ...
    if(!isAdmin_) {
      return NextResponse.json({message: "Not authorized for this route"}, {status: 401});
    }
  }
...
export const config = {
  matcher: ["/login", "/signup", "/client", "/admin", "/admin/:path*", "/client/:path*", "/api/:path*"],
};
